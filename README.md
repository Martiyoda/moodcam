...
E-motion es un proyecto educativo que convierte emociones detectadas con camara y voz en decisiones artisticas para un brazo Arduino/ESP32.

El modo recomendado para clase y desarrollo es local: la web y el backend se arrancan con `npm run dev`, HiveMQ actua como broker MQTT externo y la ESP32 se programa desde la carpeta `arduino`.
...
El MVP actual esta preparado para probar la comunicacion con ESP32 de forma segura:

```txt
Web E-motion -> HiveMQ -> AI Bridge/OpenAI -> HiveMQ -> ESP32 o simulador
```

Comandos principales:

```bash
npm run dev           # web + backend local
npm run demo:robot    # AI Bridge + ESP32 simulator
npm run demo:session  # publica una sesion falsa por HiveMQ
```

See:

- [Arquitectura](docs/ARCHITECTURE.md)
- [Arranque local y despliegue opcional](docs/RUN_LOCAL_AND_DEPLOY.md)
- [Guia educativa](docs/EDUCATIONAL_OVERVIEW.md)
- [Robot y calibracion](docs/ROBOT_SETUP.md)
- [Contrato AI Bridge](docs/mvp-ai-bridge.md)
- [Runbook MVP ESP32](docs/runbooks/mvp-esp32-test.md)
...
- **Modo local sencillo** — Web, backend y demos se pueden arrancar desde `npm` para trabajar en clase sin depender de despliegues externos.
...
| **Publicacion opcional** | Vercel u otro hosting para compartir la web |

## 📁 Project Structure

```txt
project/
├── src/                    # Web React: camara, voz, MQTT y paneles
├── server/                 # Backend local, AI Bridge, validacion y simulador
├── arduino/                # Sketch Arduino/ESP32 y configuracion del robot
├── packages/contracts/     # Topics y payloads MQTT compartidos
├── docs/                   # Arquitectura, arranque, robot y handoff
├── strokes/                # Recetas artisticas aprobadas
├── examples/               # Entradas y pruebas manuales
├── scripts/                # Comandos de desarrollo y demo
├── public/models/          # Modelos de deteccion local
├── package.json
└── vite.config.js
```

El sketch Arduino oficial actual esta documentado en [arduino/README.md](arduino/README.md).
...
Abre `http://127.0.0.1:5173/`. Desde ahi puedes usar la experiencia web, activar MQTT y entrar en el panel de calibracion.
...
La publicacion externa no es necesaria para clase ni calibracion. El modo recomendado sigue siendo local con HiveMQ como broker externo.
